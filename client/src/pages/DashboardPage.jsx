...
function DashboardPage() {
  ...
  return (
    // MAIN CONTAINER: Nền tối Trello #1d2125
    <div className="flex flex-col h-full bg-gray-50 dark:bg-[#1d2125]">
            ...
            {statCards.map((stat) => (
                <div key={stat.title} className={`p-6 rounded-xl shadow-sm border transition-transform hover:-translate-y-1 ${stat.bg} ${stat.border}`}>
                    <div className="flex justify-between items-start">
                        ...
                        {/* Icon nền tối nhẹ */}
                        <div className={`p-2 rounded-lg bg-white/60 dark:bg-black/20 ${stat.color}`}>
                ...
                <div className="bg-white dark:bg-[#22272b] p-6 rounded-xl shadow-sm border border-gray-100 dark:border-white/10">
                    ...
                    <form onSubmit={handleCreateBoard} className="flex gap-3">
                        <input
                            ...
                            // Input nền tối hơn card (#1d2125), border mờ
                            className="flex-grow px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 bg-gray-50 dark:bg-[#1d2125] text-gray-900 dark:text-[#b6c2cf] transition-all outline-none"
                        />
                        <button
                            type="submit"
                            disabled={isCreating}
                            ...
                            {boards.map((board) => {
                                ...
                                return (
                                    <Link
                                        key={board._id}
                                        to={`/board/${board._id}`} 
                                        // Thẻ Board màu #22272b, border mờ
